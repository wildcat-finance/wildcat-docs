---
description: What it means for a market to be pending, and the resulting impact.
---
...
A market that goes below the reserve ratio - however it does so - is _pending_.

## Related Market Parameters

As well as the reserve ratio, another two parameters are provided when creating a Wildcat market:

* **The grace period**: the time (measured in seconds) that a market is permitted to be pending for without adverse effects, and
* **The penalty APR**: the interest rate that applies over and above the base market APR and protocol fee (if present) in the event that a market stays pending for longer than the grace period.

## How Pending Triggers

Associated with each market is an internal number called the '_grace tracker_' - this is a number that starts counting up from zero after the block in which a market becomes pending, and back down towards zero after the block in which a market is cured of its pending state.
...
* A market with a 5 day grace period becomes pending for the first time, and the grace tracker begins counting up from zero.
* The borrower takes 7 days to cure the market of its pending.
* Once the pending is cured, the market calculates that 2 days of penalty APR must be applied and adjusts the scaling factor of market tokens accordingly.
...
## Unclaimed/Pending Withdrawals & Pending
...
* The market now has 50,000 USDC in reserves against a supply of 800,000 USDC. This means that the new reserve ratio is 6.25%, and the market is immediately pending.
* The borrower needs to return 110,000 USDC to the market reserves in order to cure the pending state.
...
* Against a supply of 750,000 USDC, this means that the temporary reserve ratio is 36% rather than the 20% we would 'expect' to see against the remaining supply: the market will remain pending until this 36% has been met.
